...
lm(Diameter ~ Limb, data = lizards) %>% 
  augment %>% ggplot(aes(x = Limb)) +
...
sim3
sim3 %>% ggplot(aes(x = x1, y = y, color = x2)) + 
  geom_point()
...
model_comparison_data %>% 
  ggplot(aes(x = x1, color = Model)) + facet_wrap(~x2) +
